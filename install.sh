...
if [[ -f "$LOCAL_SOURCE" ]]; then
  ...
  echo "[*] bersihin.py tidak ada di folder lokal; mengunduh dari GitHub..."
  ...
  "$PYTHON_BIN" -m py_compile "$TEMP_SOURCE"
  SOURCE="$TEMP_SOURCE"
fi
...
cp "$SOURCE" "$INSTALL_DIR/bersihin.py"
chmod 644 "$INSTALL_DIR/bersihin.py"
